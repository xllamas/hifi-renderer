...
#
# These units fail silently rather than loudly, which is why they are worth
# testing without a phone, a DAC and a media server in the loop: PcmDecoder and
# DsdDecoder read the wrong way round produce noise, and a ToneSource that does not loop in
...
${CXX:-c++} -std=c++17 -Wall -Wextra \
    -I "$cpp" -I "$here/shim" \
    -o "$out/dsd_decoder_test" \
    "$here/dsd_decoder_test.cpp" "$cpp/decode/DsdDecoder.cpp"
